...
class GeminiEmbedder:
    """Custom embedder using Gemini API for embeddings (fallback to hash-based if not supported)"""
    def __init__(self, api_key: str, model: str = "gemini-2.5-flash-lite"):
        self.api_key = api_key
        self.model = model
        self.client = GeminiClient(api_key=api_key)
        self.embedding_dim = 384  # Default embedding dimension
        self.dimensions = 384  # Required by Qdrant

    def get_embedding(self, text: str):
        try:
            # Gemini does not provide native embeddings, fallback to hash-based
            import hashlib
            import numpy as np
...
def init_qdrant():
    ...
    try:
        ...
        # Using custom Gemini embedder
        vector_db = Qdrant(
            collection=COLLECTION_NAME,
            url=st.session_state.qdrant_url,
            api_key=st.session_state.qdrant_api_key,
            embedder=GeminiEmbedder(api_key=st.session_state.gemini_api_key, model="gemini-2.5-flash-lite")
        )
        return vector_db
    except Exception as e:
        ...
        with st.spinner('📤 Loading documents into knowledge base...'):
            try:
                ...
                if '403' in str(e) or 'forbidden' in str(e).lower():
                    st.error("❌ Gemini API key is invalid, expired, or lacks permission. Please check your API key and try again.")
                else:
...
def main():
    ...
    with st.sidebar:
        ...
        gemini_key = st.text_input(
            ...
            help="Enter your Gemini API key (for both LLM and embeddings)"
        )
        if gemini_key:
        ...
        if all([st.session_state.gemini_api_key, st.session_state.vector_db]):
                        ...
                        try:
                            ...
                            if knowledge_base:
                                ...
                                legal_researcher = Agent(
                                    ...
                                    model=Gemini(id="gemini-2.5-flash-lite", api_key=st.session_state.gemini_api_key),
    ...
    # Main content area
    if not all([st.session_state.gemini_api_key, st.session_state.vector_db]):
